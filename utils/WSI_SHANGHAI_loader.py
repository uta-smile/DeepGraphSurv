...
DATA_DIR = os.path.join(BASE_DIR, 'data/WSI/shanghai_surv/vgg_4096/')

DATA_COMPRESS_DIR = os.path.join(BASE_DIR, 'data/WSI/shanghai_surv/vgg_pca/')
if not os.path.exists(DATA_COMPRESS_DIR):
    os.makedirs(DATA_COMPRESS_DIR)

TRAIN_DIR = os.path.join(DATA_COMPRESS_DIR, 'Train')
TEST_DIR = os.path.join(DATA_COMPRESS_DIR, 'Test')
if not os.path.exists(TRAIN_DIR):
    os.makedirs(TRAIN_DIR)
if not os.path.exists(TEST_DIR):
    os.makedirs(TEST_DIR)

RAW_FILES = glob.glob(DATA_DIR+'*.npz')
...
def reduce_dimension(pca_data_dir):
    if not os.path.exists(pca_data_dir):
        os.makedirs(pca_data_dir)

    train_test_split_wsi()


def train_test_split_wsi(train_ratio=0.6,
                         test_ratio=0.4):

    # prepare the shuffle list
    num_all_wsi = len(RAW_FILES)
    idx_list = np.arange(0, num_all_wsi)
    random.shuffle(idx_list)

    train_list = idx_list[:int(train_ratio * num_all_wsi)]
    test_list = idx_list[-int(test_ratio * num_all_wsi):]
    val_list = idx_list[int(train_ratio * num_all_wsi): -int(test_ratio * num_all_wsi)]

    train_patch_set, test_patch_set, val_patch_set = [], [], []

    # get all patches data into one single matrix and then do PCA
    for idx, wsi_dir in enumerate(RAW_FILES):
        patient = wsi_dir.split("/")[-1][:6]
        print("reading wsi at %s froml patient %s" % (wsi_dir, patient))
        raw_data = np.load(wsi_dir)
        current_wsi = raw_data['vgg_features']
        # node_img_path = raw_data['img_path']

        if idx in train_list:
            train_patch_set += [current_wsi]
        elif idx in test_list:
            test_patch_set += [current_wsi]
        elif train_ratio + test_ratio < 1.0 and idx in val_list:
            val_patch_set += [current_wsi]
        else:
            ValueError("idx not in any set")

    train_data = np.stack(train_patch_set).reshape()
    test_data = np.stack(test_patch_set)
    if len(val_patch_set) > 0:
        val_data = np.stack(val_patch_set)
...
def load(dataset_name='processed_500maxnode'):
    ...
    if len(os.listdir(processed_data_dir)) != 0:
        ...
        reduce_dimension(DATA_COMPRESS_DIR)
