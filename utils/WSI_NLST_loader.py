...
DATA_DIR = os.path.join(BASE_DIR, 'data/WSI/NLST_128')
Train_dir_LUAD = os.path.join(DATA_DIR, 'NLST_ADC/vgg_pca/Train/')
Test_dir_LUAD = os.path.join(DATA_DIR, 'NLST_ADC/vgg_pca/Test/')

Train_dir_LUSC = os.path.join(DATA_DIR, 'NLST_SCC/vgg_pca/Train/')
Test_dir_LUSC = os.path.join(DATA_DIR, 'NLST_SCC/vgg_pca/Test/')
...
TRAIN_FILES_LUAD = glob.glob(Train_dir_LUAD+'*.np[yz]')
TEST_FILES_LUAD = glob.glob(Test_dir_LUAD+'*.npz')
TRAIN_FILES_LUSC = glob.glob(Train_dir_LUSC+'*.npz')
TEST_FILES_LUSC = glob.glob(Test_dir_LUSC+'*.npz')

TRAIN_FILES = TRAIN_FILES_LUAD + TRAIN_FILES_LUSC
TEST_FILES = TEST_FILES_LUAD + TEST_FILES_LUSC

ALL_FILES = TRAIN_FILES + TEST_FILES
# TRAIN_FILES = map(lambda x: x.split('/')[-1], TRAIN_FILES)
# TEST_FILES = map(lambda x: x.split('/')[-1], TEST_FILES)

MIN_NUM_POINT = 5    # number of point you extract from raw files
MAX_NUM_POINT = 1000
...
def get_shards(data_dir, file_list, shard_size, istraining):
    ...
    for shard_idx in range(shard_num):
        ...
        for fn in shard_files_idxs:
            ...
            if curr_type == 'NLST_ADC':
                # NLST_ADC -> class 0, NLST_SCC -> class 1
                current_label = 0
            else:
                current_label = 1
