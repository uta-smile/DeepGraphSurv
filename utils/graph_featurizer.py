...
def get_graph_featurize(mol):
    # Get the node features
    idx_nodes = [(a.GetIdx(), atom_features(a)) for a in mol.GetAtoms()]
    idx_nodes.sort()  # Sort by ind to ensure same order as rd_kit
    idx, nodes = list(zip(*idx_nodes))
    nodes_features = np.vstack(nodes)    # only atom feature

    atoms_feature_rd_idx = {}
    bond_feature = {}
    for a in mol.GetAtoms():
        atoms_feature_rd_idx[a.GetIdx()] = atom_features(a)
        bond_feature[a.GetIdx()] = []

    b_feature_size = 0
    for b in mol.GetBonds():
        # atom's bond feature, high degree atom has more features
        bond_feature[b.GetBeginAtom().GetIdx()].append(bond_features(b))
        bond_feature[b.GetEndAtom().GetIdx()].append(bond_features(b))
        b_feature_size = max(bond_features(b).shape[0], b_feature_size)
    ...
    canon_adj_list = [[] for _ in range(len(nodes_features))]
    for edge in edge_list:
        canon_adj_list[edge[0]].append(edge[1])
        canon_adj_list[edge[1]].append(edge[0])

    return nodes_features.astype(np.float32), canon_adj_list
