...
IMG_DIR = os.path.join(LOG_DIR, 'image_save6')
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        ops = {'pointclouds_pl': data_pl,
               ...
               'an_out': end_points['an_out'],
               # 'res_graph': end_points['res_graph'],
               'loss': loss,
               # 'node_risk': end_points['node_pred'],
               'train_op': train_op,
        ...
        # train model
        max_repeat = 1
        for trial_id, lr in enumerate([0.005]):
            ...
            init = tf.global_variables_initializer()
            sess.run(init,
                     {is_training_pl: True, learning_rate_pl: lr})
...
def save_graph_feature(result_dict, epoch, istrain=True):
    ...
    with open(os.path.join(FEATURE_DIR, csv_file), 'a') as csvfile:
        ...
        for idx, p in enumerate(features.keys()):
            p_idx = np.where(pid == p)[0]
            writer.writerow([p] + [status[p_idx]] + [time[p_idx]] + features[p].tolist())
...
def display_dist(dist, epoch):

    import seaborn as sns
    sns.set(color_codes=True)
    # dist is the ndarray prob,s sum == 1
    dist = np.reshape(dist, (-1))
    print("attention on %s patch" % dist.shape[0])

    sns_plot = sns.distplot(dist)
    fig = sns_plot.get_figure()
    fig.savefig(os.path.join(IMG_DIR, 'dist_9389_ep%s.png' % epoch))
...
def train_one_epoch(epoch, sess, ops, lr):
    ...
    data, laplacian, size_x, wsi_name, node_img = prepare_data.prepare_train_data(DATA_NAME, DATASET_NAME)
    ...
    all_features, all_attn, all_status, all_time, all_pid, all_wsi_name = [], [], [], [], [], []
    ...
    while end_idx <= file_size:
        ...
        if 1 not in status.tolist() or 0 not in status.tolist():
            ...
            step, _, loss_val, pred_val, attn, an_out, graph_feature = sess.run(
                [
                    ops['step'],
                    ops['train_op'],
                    ops['loss'],
            ...
            if np.any(np.array(['11456' in p for p in wsi_sel.tolist()])):
                print('find it !')
                idx_batch = np.where(wsi_sel == '11456.npz')
                # sel_res_graph = np.squeeze(res_graph[idx_batch])
                # sel_node_risk = np.squeeze(node_risk[idx_batch])
                sel_size_x = batch_size_x[idx_batch][0]
                sel_node_img = batch_node_img[idx_batch]
                sel_attn = np.squeeze(attn[idx_batch])
                print('total we have %s patch for WSI %s named 9389' % (sel_size_x, idx_batch))
                original_graph = np.squeeze(batch_lap[idx_batch])

                graph = original_graph[: sel_size_x, : sel_size_x]
                # res_graph = sel_res_graph[: sel_size_x, : sel_size_x]
                # node_risk = sel_node_risk[: sel_size_x]
                sel_attn = sel_attn[: sel_size_x]
                if epoch < 5 or epoch % 5 == 0:
                    # plot_graph_on_WSI(graph, sel_node_img, sel_attn, epoch, res_graph)

                    plot_graph_on_WSI(graph, sel_node_img, sel_attn, epoch)
                    # display_dist(sel_attn, epoch)
    ...
    all_attn = np.squeeze(np.vstack(all_attn))
...
def read_wsi_slic(svs_path):
    import openslide as slide
    import cv2
    wsi_image = slide.OpenSlide(svs_path)
    level_used = wsi_image.level_count - 1
    rgb_image_pil = wsi_image.read_region((0, 0), level_used,
                                          wsi_image.level_dimensions[level_used])
    rgb_image = np.array(rgb_image_pil)

    ori_image = cv2.resize(rgb_image, (0, 0), fx=0.50, fy=0.50)
    new_img = ori_image[:, :, 0:3]
    # scale = wsi_image.level_downsamples[level_used] * 2
    # hed_img = separate_stains(ori_image[:, :, :-1], hed_from_rgb)
    return new_img, wsi_image.dimensions


def plot_graph_on_WSI(graph, node_img, attn, epoch, res_graph=None):

    import matplotlib.pyplot as plt

    wsi_path = os.path.join(os.environ["HOME"], 'AGCN_tf/data/WSI/9389', '11456.svs')
    assert os.path.exists(wsi_path)
    wsi_img, wsi_dim = read_wsi_slic(wsi_path)
    wsi_img = wsi_img.astype(np.float32)

    # wsi_img = wsi_img.astype(np.uint8)
    # plt.figure(epoch)
    # plt.imshow(wsi_img, aspect="auto")

    # resize patch coord
    height, width = wsi_img.shape[0], wsi_img.shape[1]
    ratio_row, ratio_col = height / float(wsi_dim[1]), width / float(wsi_dim[0])

    # original_adj_list = get_adj_list(graph)
    # res_adj_list = get_adj_list(res_graph)

    # parse node info
    node_img_list = node_img[0]
    node_coords = []
    all_row = []
    all_col = []
    for idx, patch_name in enumerate(node_img_list):
        # print('processing patch %s' % patch_name)
        row = patch_name.decode("utf-8").split('-')[0]
        row_coord = int(int(row[4:]) * ratio_row)
        all_row.append(row_coord)
        col = patch_name.decode("utf-8").split('-')[1]
        col_coord = int(int(col[4:]) * ratio_col)
        all_col.append(col_coord)
        patch_coord = [row_coord, col_coord]
        node_coords.append(patch_coord)

    attn = np.abs(attn)
    norm_attn = [a / np.max(attn) for a in attn.tolist()]

    max_attn = np.max(norm_attn)
    min_attn = np.min(norm_attn)
    threshold = (max_attn - min_attn) * 0.8 + min_attn

    # plot node
    for p in range(len(norm_attn)):
        attn_value = norm_attn[p]
        x = node_coords[p][0]
        y = node_coords[p][1]
        if attn_value > threshold:
            ...
            color = 'y'
        plt.scatter(x=[y], y=[x], c=color, s=20)
    plt.savefig(os.path.join(IMG_DIR, 'wsi_attention_dot_ep%s.png' % epoch))
    plt.close()

    patch_row = int(2000 * ratio_row)
    patch_col = int(2000 * ratio_col)

    # mask the patch
    # normalize attention
    # wsi_img_mask = np.reshape(wsi_img, (height, width, 3)).astype(np.float32)
    # wsi_img = wsi_img.astype(np.float32)
    for p in range(attn.shape[0]):
        if norm_attn[p] > 0.9:
            attn_value = attn[p]
            x = node_coords[p][0]
            y = node_coords[p][1]
            # recover to original
            # wsi_img[y: y + patch_col, x: x + patch_row, :] *= 2.0
            # add attention
            wsi_img[y: y + patch_col, x: x + patch_row, 2] *= attn_value
            # patch_masked = patch * attn_value
            # wsi_img_mask[x: x + patch_row, y: y + patch_col] = patch_masked.astype(np.uint8)

    plt.figure(epoch)
    plt.imshow(wsi_img.astype(np.uint8), aspect="auto")

    plt.savefig(os.path.join(IMG_DIR, 'wsi_attention_map_ep%s.png' % epoch))
    plt.close()

    print("plot is done!")


def get_adj_list(laplacian):
    n_node = laplacian.shape[0]
    adj_list = []
    for n_idx in range(n_node):
        node_connect = np.abs(np.squeeze(laplacian[n_idx]))
        threshold = np.sort(node_connect).item(int(0.95*np.shape(node_connect)[0]))
        node_connect_filter = np.where(node_connect > threshold)
        adj_list.append(node_connect_filter)
    return adj_list
...
def draw_curve(preds, status, time, median_fromtrain, epoch, save_fig=True):
...
if __name__ == "__main__":
    ...
    # import matplotlib.pyplot as plt
    #
    # wsi_path = os.path.join(os.environ["HOME"], 'AGCN_tf/data/WSI/9389', '9389.svs')
    # assert os.path.exists(wsi_path)
    # wsi_img, wsi_dim = read_wsi_slic(wsi_path)
    # plt.figure(1)
    # wsi_img = wsi_img.astype(np.float32) * 0.2
    # wsi_img = wsi_img.astype(np.uint8)
    # plt.imshow(wsi_img, aspect="auto")
    #
    # wsi_img = wsi_img.astype(np.float32)
    # wsi_img[280: 280 + 131, 290: 290 + 131, :] *= 5
    # wsi_img[280: 280 + 131, 290: 290 + 131, 2] *= 0.1
    #
    # plt.figure(0)
    # plt.imshow(wsi_img.astype(np.uint8), aspect="auto")
    # plt.show()
